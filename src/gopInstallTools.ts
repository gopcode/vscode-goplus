...
export async function installAllTools(updateExistingToolsOnly: boolean = false) {
	...
	let allTools = getConfiguredTools();

	// exclude tools replaced by alternateTools.
	const alternateTools: { [key: string]: string } = getGoPlusConfig().get('alternateTools');
...
export async function installTool(
	...
	if (!modulesOn) {
		importPath = getImportPath(tool);
	} else {
		importPath = getImportPathWithVersion(tool, tool.version);
	}
...
Run "go get -v ${getImportPath(tool)}" to install.`;
